...
<head>
    ...
    <script>
                ...
                if (error !== false) {
                    ...
                    if (typeof data.fetch === 'object') {
                        ...
                        for (database in data.fetch) {
                            ...
                            out += '<button class="btn btn-secondary" type="button" onclick="tablesCreate(\'' + <?php print $beyond->prefix; ?>base64encode(database) + '\', false);">Create table';
                            out += '</button>';
                            out += '</div>';
                            ...
                            for (table in data.fetch[database]) {
                                ...
                                out += '</span>';
                                if (!internal) {
                                    out += '<span class="tableItemName" onclick="tablesView(\'' + <?php print $beyond->prefix; ?>base64encode(database) + '\', \'' + <?php print $beyond->prefix; ?>base64encode(data.fetch[database][table]) + '\', false);">';
                                } else {
                                    out += '<span class="tableItemName" onclick="tablesView(\'' + <?php print $beyond->prefix; ?>base64encode(database) + '\', \'' + <?php print $beyond->prefix; ?>base64encode(data.fetch[database][table]) + '\', true);">';
                                }
                                ...
                                if (!internal) {
                                    out += '  <i class="fas fa-pen ml-1" onclick="tablesEdit(\'' + <?php print $beyond->prefix; ?>base64encode(database) + '\', \'' + <?php print $beyond->prefix; ?>base64encode(data.fetch[database][table]) + '\', false);"></i>';
                                    out += '  <i class="fas fa-trash ml-1" onclick="tablesDrop(\'' + <?php print $beyond->prefix; ?>base64encode(database) + '\',\'' + <?php print $beyond->prefix; ?>base64encode(data.fetch[database][table]) + '\');"></i>';
                                } else {
                                    out += '  <i class="fas fa-eye ml-1" onclick="tablesEdit(\'' + <?php print $beyond->prefix; ?>base64encode(database) + '\', \'' + <?php print $beyond->prefix; ?>base64encode(data.fetch[database][table]) + '\', true);"></i>';
                                }
        ...
        function tablesDrop(databaseBase64, tableBase64, fromModal = false) {
            if (fromModal === false) {
                $('#dialogTablesDrop .modal-body').html('Drop table <b>' + <?php print $beyond->prefix; ?>base64decode(tableBase64) + '</b> from database <b>' + <?php print $beyond->prefix; ?>base64decode(databaseBase64) + '</b>');
                $('#dialogTablesDrop').data('table', <?php print $beyond->prefix; ?>base64decode(tableBase64)).data('database', <?php print $beyond->prefix; ?>base64decode(databaseBase64)).modal('show');
                return false;
            }
            <?php print $beyond->prefix; ?>api.beyondTables.drop({
                'database': <?php print $beyond->prefix; ?>base64decode(databaseBase64),
                'table': <?php print $beyond->prefix; ?>base64decode(tableBase64)
            }, function (error, data) {
                if (error !== false) {
                    message('Error: ' + error);
                } else {
                    if (data.drop === true) {
                        ...
                        message('Drop table [' + <?php print $beyond->prefix; ?>base64decode(tableBase64) + '] from database [' + <?php print $beyond->prefix; ?>base64decode(databaseBase64) + '] failed: ' + data.drop);
                    }
                }
            });
        }
        ...
        function tablesCreate(databaseBase64, fromModal = false, tableName, fieldName, kind, index, allowNull, defaultValue) {
            if (fromModal === false) {
                $('#dialogTablesCreateTitle').html('Create table in database <b>' + <?php print $beyond->prefix; ?>base64decode(databaseBase64) + '</b>');
                $('#addTableName').val('');
                ...
                $('#dialogTablesCreate').data('database', <?php print $beyond->prefix; ?>base64decode(databaseBase64)).modal('show').on('shown.bs.modal', function (e) {
                    $('#addTableName').focus();
                });
                return false;
            }
            ...
            <?php print $beyond->prefix; ?>api.beyondTables.create({
                ...
                if (error !== false) {
                    ...
                    if (data.create === true) {
                        ...
                        message('Create table [' + tableName + '] in database [' + <?php print $beyond->prefix; ?>base64decode(databaseBase64) + '] failed: ' + data.create);
                    }
                }
            });
        }
            ...
            <?php print $beyond->prefix; ?>api.beyondTables.info({
                ...
                if (error !== false) {
                    ...
                    if (data.info !== false) {
                        ...
                        editDatabase = <?php print $beyond->prefix; ?>base64decode(databaseBase64);
                        editTable = <?php print $beyond->prefix; ?>base64decode(tableBase64);
                        ...
                        message('Fetch table [' + <?php print $beyond->prefix; ?>base64decode(tableBase64) + '] details from database [' + <?php print $beyond->prefix; ?>base64decode(databaseBase64) + '] failed');
                    }
                }
            });
        }
            ...
            var html =
                ...
                (
                    ((fieldCount < 2) || (internalTable === true)) ? '' :
                        '<i class="fas fa-trash mt-1" style="cursor:pointer;" onclick="tablesColumnDrop(\'' + <?php print $beyond->prefix; ?>base64encode(fieldName) + '\');"></i>'
                ) +
                '  </td>' +
                '</tr>';
            ...
            <?php print $beyond->prefix; ?>api.beyondTables.columnAdd(data, function (error, data) {
                if (error !== false) {
                    ...
                    if (data.columnAdd === true) {
                        ...
                        tablesEdit(<?php print $beyond->prefix; ?>base64encode(editDatabase), <?php print $beyond->prefix; ?>base64encode(editTable), false);
                    } else {
        ...
        function tablesColumnDrop(fieldNameBase64, fromModal = false) {
            if (fromModal === false) {
                $('#dialogTablesDropField .modal-body').html('Drop column <b>' + <?php print $beyond->prefix; ?>base64decode(fieldNameBase64) + '</b> of table <b>' + editTable + '</b> in database <b>' + editDatabase + '</b>');
                $('#dialogTablesDropField').data('fieldName', <?php print $beyond->prefix; ?>base64decode(fieldNameBase64)).modal('show');
                return false;
            }
            <?php print $beyond->prefix; ?>api.beyondTables.columnDrop({
                'database': editDatabase,
                'table': editTable,
                'field': <?php print $beyond->prefix; ?>base64decode(fieldNameBase64)
            }, function (error, data) {
                if (error !== false) {
                    message('Error: ' + error);
                } else {
                    if (data.columnDrop === true) {
                        ...
                        message('Drop column [' + <?php print $beyond->prefix; ?>base64decode(fieldNameBase64) + '] from table [' + editTable + '] of database [' + editDatabase + '] failed: ' + data.columnDrop);
                    }
                }
            });
        }
            ...
            <?php print $beyond->prefix; ?>api.beyondTables.info({
                ...
                if (error !== false) {
                    ...
                    if (data.info !== false) {
                        ...
                        viewDatabase = <?php print $beyond->prefix; ?>base64decode(databaseBase64);
                        viewTable = <?php print $beyond->prefix; ?>base64decode(tableBase64);
            ...
            <?php print $beyond->prefix; ?>api.beyondTables.rowCount({
                ...
                if (error !== false) {
                    ...
                    if (data.rowCount !== false) {
                        ...
                        message('Fetch row count from table [' + <?php print $beyond->prefix; ?>base64decode(tableBase64) + '] of database [' + <?php print $beyond->prefix; ?>base64decode(databaseBase64) + '] failed');
                    }
                }
            });
        }
                ...
                if (error !== false) {
                    ...
                    if (data.loadData !== false) {
                        for (rowNo in data.loadData) {
                            ...
                            var rowData = <?php print $beyond->prefix; ?>base64encode(JSON.stringify(data.loadData[rowNo]));
                            cols += '<td align=center nowrap>';
                            if (((viewAutoColumn !== '') || (viewPrimaryColumn !== '')) && (viewInternalTable !== true)) {
                                cols += '<i class="fas fa-pen mt-1" style="cursor:pointer;" onclick="console.log(); tablesViewRowEdit(\'' + <?php print $beyond->prefix; ?>base64encode(keyValue) + '\', \'' + rowData + '\');"></i>';
                                cols += '<i class="fas fa-trash mt-1 ml-2" style="cursor:pointer;" onclick="tablesViewRowDelete(\'' + <?php print $beyond->prefix; ?>base64encode(keyValue) + '\', false);"></i>';
                            }
                            cols += '</td>';
                            $('#viewCells tbody').append('<tr>' + cols + '</tr>');
                        }
                    } else {
                        message('Fetch rows from table [' + <?php print $beyond->prefix; ?>base64decode(tableBase64) + '] of database [' + <?php print $beyond->prefix; ?>base64decode(databaseBase64) + '] failed');
                    }
                }
            });
        }
            ...
            <?php print $beyond->prefix; ?>api.beyondTables.addData({
                ...
                if (error !== false) {
                    ...
                    if (data.addData !== false) {
                        ...
                        message('Add rows to table [' + <?php print $beyond->prefix; ?>base64decode(tableBase64) + '] of database [' + <?php print $beyond->prefix; ?>base64decode(databaseBase64) + '] failed');
                    }
                }
            });
        }
        ...
        function tablesViewRowEdit(primaryValueBase64 = false, dataJsonBase64 = false) {
            if (dataJsonBase64 !== false) {
                dataJson = JSON.parse(<?php print $beyond->prefix; ?>base64decode(dataJsonBase64));
                ...
                $('#dialogViewRowEdit .modal-body').html(
                    '<div class="mb-4">Modify row with primary key <b>' + <?php print $beyond->prefix; ?>base64decode(primaryValueBase64) + '</b> from table <b>' + viewTable + '</b> in database <b>' + viewDatabase + '</b></div>' +
                    data
                );
                $('#dialogViewRowEdit').data('primaryValue', <?php print $beyond->prefix; ?>base64decode(primaryValueBase64)).modal('show');
            ...
            <?php print $beyond->prefix; ?>api.beyondTables.modifyData({
                ...
                'value': <?php print $beyond->prefix; ?>base64decode(primaryValueBase64)
            }, function (error, data) {
                if (error !== false) {
                    message('Error: ' + error);
                } else {
                    if (data.modifyData !== false) {
                        ...
                        message('Modify rows with primary key [' + <?php print $beyond->prefix; ?>base64decode(primaryValueBase64) + '] from table [' + <?php print $beyond->prefix; ?>base64decode(tableBase64) + '] of database [' + <?php print $beyond->prefix; ?>base64decode(databaseBase64) + '] failed');
                    }
                }
            });
        }
        ...
        function tablesViewRowDelete(primaryValueBase64, fromModal = false) {
            if (fromModal === false) {
                $('#dialogViewRowDelete .modal-body').html('<div class="mb-4">Delete row with primary key <b>' + <?php print $beyond->prefix; ?>base64decode(primaryValueBase64) + '</b> from table <b>' + viewTable + '</b> in database <b>' + viewDatabase + '</b></div>');
                $('#dialogViewRowDelete').data('primaryValue', <?php print $beyond->prefix; ?>base64decode(primaryValueBase64)).modal('show');
                return false;
            }
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-success" type="button"
                            ...
                            <?php print $beyond->prefix; ?><?php print $beyond->prefix; ?>base64encode($('#dialogTablesCreate').data('database')),
                                true,
                                $('#addTableName').val(),
                                $('#addTableColumnName').val(),
    ...
    <div class="modal fade" id="dialogTablesDrop" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button class="btn btn-danger" type="button"
                            onclick="tablesDrop(<?php print $beyond->prefix; ?>base64encode($('#dialogTablesDrop').data('database')), <?php print $beyond->prefix; ?>base64encode($('#dialogTablesDrop').data('table')), true);">
                        Drop table
                    </button>
                    <button class="btn btn-success" type="button" data-dismiss="modal">Cancel</button>
                </div>
